...
pub fn wkt_polygon_union(geom1: &String, geom2: &String) -> Result<String, JsValue> {
    ...
    let poly1 = pair.geom1
        .into_polygon()
        .unwrap_or_else(|| panic![format!("Could not convert geometry to polygon: {}", geom1)]);

    if !poly1.validate() {
        return Err(json_errors::boolean_geometry_errors::invalid_geom(&"invalid polygon".into()))
    }

    let poly2 = pair.geom2
        .into_polygon()
...
pub fn wkt_multi_polygon_polygon_union(geom1: &String, geom2: &String) -> String {
    ...
    let poly1 = pair.geom1
        .into_multi_polygon()
        .unwrap_or_else(|| panic![format!("Could not convert geometry to multipolygon: {}", geom1)]);
    let poly2 = pair.geom2
        .into_polygon()
        .unwrap_or_else(|| panic![format!("Could not convert geometry to polygon: {}", geom2)]);
...
pub fn wkt_multi_polygon_union(geom1: &String, geom2: &String) -> String {
    ...
    let poly2 = pair.geom2
        .into_multi_polygon()
        .unwrap_or_else(|| panic![format!("Could not convert geometry to multipolygon: {}", geom2)]);
...
where
    T: num_traits::Float + std::str::FromStr + std::default::Default,
{
    let wkt_geom1: Wkt<T> = Wkt::from_str(geom1).ok()
    ...
    let wkt_geom2: Wkt<T> = Wkt::from_str(geom2).ok()
        .unwrap_or_else(|| panic![format!("WKT geometry not recognized: {}", geom1)]);
...
mod tests {
    ...
    #[test]
    #[should_panic(expected = "WKT geometry not recognized: BAD((0 0,10 0,10 10,0 10,0 0),(2 2,6 2,6 6,2 6,2 2))")]
